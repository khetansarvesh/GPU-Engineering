...
- It helps you understand about memory hierarchy — Learn HBM → L2 → shared memory → registers. And then you should see why FlashAttention is fast (it tiles into SRAM to avoid HBM round-trips).
- It introduces Matmul kernel implementations. Understand thread blocks, warps, coalesced access, Tensor Cores. After this see the Flash Attention implementation and see if you can understand that or not.
- Try to implement more kernel operations.
